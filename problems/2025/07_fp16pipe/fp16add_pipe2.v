...
// large_m, small_m, sticky, e_diff, large_e, op_sub
...
always @(*) begin
    large_aligned = {1'b0, large_m_d, 16'b0};
    small_aligned = ({1'b0, small_m_d, 16'b0} >> e_diff_d) | {27'b0, sticky_d};
    if (op_sub_d) begin
...
// norm_e, norm_m, large_s

reg [26:0] norm_m_d;
reg signed [6:0] norm_e_d;
...
wire g = norm_m_d[15];
wire r = norm_m_d[14];
wire s = |norm_m_d[13:0];

wire rnd_inc = g & (l | r | s);

wire [11:0] round_m = {1'b0, norm_m_d[26:16]} + rnd_inc;
...
always @(*) begin
    ...
    if (round_m[11]) begin
         final_e = norm_e_d[4:0] + 1;
         final_m = round_m[10:1];
    end else begin
         final_e = norm_e_d[4:0];
         final_m = round_m[9:0];
    end
    ...
    if (norm_e <= 0 || sum == 0) begin
        // FTZ
        o_z = {large_s_d, 15'd0};
    end else if (norm_e_d >= 31) begin
        // Overflow: Infinity
        o_z = {large_s_d, 5'h1F, 10'd0};
    end else begin
        // Normal
        o_z = {large_s_d, final_e, final_m};
    end
end
